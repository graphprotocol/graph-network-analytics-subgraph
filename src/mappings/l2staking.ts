...
*/
export function handleTransferredDelegationReturnedToDelegator(
  event: TransferredDelegationReturnedToDelegator,
): void {
  let graphAccount = createOrLoadGraphAccount(event.params.indexer, event.block.timestamp)
  graphAccount.balanceReceivedFromL1Delegation = graphAccount.balanceReceivedFromL1Delegation.plus(
    event.params.amount,
  )
  graphAccount.save()
}
